...
def create_window():
    ...
    layout = [
        ...
        [sg.Text('Turma'), sg.Combo(class_room_list, key='list')],
...
def event_handler(event, values):
    ...
    if event == 'return interface':
        ...
        elif input_class_room == '':
            sg.popup('Por favor, selecione uma turma')
        else:
            connection_group.create_group({'name': input_groupname, 'class-room-id':input_class_room['id']})
